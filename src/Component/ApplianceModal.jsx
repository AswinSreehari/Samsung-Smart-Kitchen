    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center sm:items-end sm:justify-start bg-transparent pointer-events-none">
      <div
        className={`
          ...
          w-[80vw] max-w-[300px] sm:w-[390px] sm:max-w-[390px]
      ...
      >
        {/* Title + Close */}
        <div className="flex items-start justify-between pt-3 sm:pt-5 pl-3 sm:pl-6 pr-3 sm:pr-6 pb-0">
          <div>
            <div className="font-bold text-sm sm:text-lg text-black mb-1">
              {data.title}
            </div>
          ...
          <button
            className="ml-2 mt-1 text-sm sm:text-[18px] text-red-600 font-bold hover:text-red-700"
            onClick={onClose}
            aria-label="Close"
            style={{
              lineHeight: "1",
        ...
        <div className="pl-3 sm:pl-6 pb-2">
          {data.title === "BeSpoke Appliances" ? (
            <div className="flex items-start mt-2 mb-3 text-xs sm:text-[15px]">
              <TiTick color="green" size={18} className="flex-shrink-0 mt-0.5 mr-2" />
            ...
            <ul className="mt-1 mb-3">
              {data.features.map((feature, idx) => (
                <li key={idx} className="flex items-center mb-1.5 text-xs sm:text-[14px]">
                  <TiTick color="green" size={18} className="mr-2" />
                  <span>{feature}</span>
                </li>
              ))}
            </ul>
        ...
        <div className="flex flex-wrap justify-between px-2 sm:px-5 pb-4 gap-2 sm:gap-3">
          {data.images.map((imgSrc, idx) => (
            <img
              key={idx}
              src={imgSrc}
              alt={`${data.title} ${idx + 1}`}
              className="rounded-md bg-white border w-[36px] h-[28px] sm:w-[62px] sm:h-[52px] object-cover shadow"
